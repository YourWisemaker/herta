...
arithmetic.pow = function(base, exponent) {
  ...
  if (Array.isArray(base)) {
    ...
    if (Number.isInteger(exponent) && exponent >= 0) {
      ...
      let result = Array(rows).fill().map((_, i) => {
        return Array(rows).fill().map((_, j) => i === j ? 1 : 0);
      });
  ...
  if (base < 0 && Number.isInteger(exponent)) {
    return exponent % 2 === 0 ? Math.pow(-base, exponent) : -Math.pow(-base, exponent);
  }
...
arithmetic.round = function(value, decimals = 0) {
  ...
  return new Decimal(value).times(factor).round().div(factor).toNumber();
};
