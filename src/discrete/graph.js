 ...
 */
function originalCreateGraph(options = {}) { // Renamed to avoid conflict with exported create
...
export function createUndirectedGraph() {
  return create({ directed: false, weighted: true }); // create will be the wrapped one
}

export function createDirectedGraph() {
  return create({ directed: true, weighted: true }); // create will be the wrapped one
}
...
// The original create function was renamed to originalCreateGraph
export const create = function (options) { // This is the main exported create
  const graph = originalCreateGraph(options); // Call the renamed original function
  return addGraphPrototypeMethods(graph);
};

// No need for module.exports in ESM
// Individual exports are already handled
