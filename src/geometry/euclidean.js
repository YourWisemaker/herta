...
geometry.distance = function (point1, point2) {
  ...
  for (let i = 0; i < point1.length; i++) {
    sumSquares += (point2[i] - point1[i]) ** 2;
  }
...
geometry.convexHull = function (points) {
  ...
  const sortedPoints = points.slice().sort((a, b) => {
    ...
    if (angleA === angleB) {
      ...
      const distA = (a[0] - lowestPoint[0]) ** 2 + (a[1] - lowestPoint[1]) ** 2;
      const distB = (b[0] - lowestPoint[0]) ** 2 + (b[1] - lowestPoint[1]) ** 2;
      return distA - distB;
    }
...
geometry.minimumEnclosingCircle = function (points) {
  ...
  if (points.length === 2) {
    ...
    const radius = Math.sqrt(
      (points[0][0] - center[0]) ** 2
      + (points[0][1] - center[1]) ** 2
    );
    return { center, radius };
  }
