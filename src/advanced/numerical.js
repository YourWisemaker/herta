...
numerical.integrate.monteCarlo = function(f, lowerBounds, upperBounds, samples = 10000) {
  ...
  const volume = upperBounds.reduce((vol, upper, i) => 
    vol * (upper - lowerBounds[i]), 1);
  ...
  for (let i = 0; i < samples; i++) {
    ...
    const point = Array(dim).fill(0).map((_, j) => 
      lowerBounds[j] + Math.random() * (upperBounds[j] - lowerBounds[j]));
  ...
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < n; j++) {
      if (i !== j) {
        ...
        D[i][j] = (ci / cj) * Math.pow(-1, i + j) / (x_cheb[i] - x_cheb[j]);
      }
    }
  }
