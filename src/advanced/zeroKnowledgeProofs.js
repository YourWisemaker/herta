...
zeroKnowledgeProofs.discreteLogProof = function (params = {}) {
  ...
  return {
    ...
    verify(proof) {
      const {
        publicValue, commitment, challenge, response
      } = proof;
...
zeroKnowledgeProofs.rangeProof = function (params = {}) {
  ...
  return {
     ...
     * @returns {boolean} - True if the proof is valid
     */
    verify(proof, upperBound) {
      const {
        bitCommitments, challenges, responses, numBits
      } = proof;
...
zeroKnowledgeProofs.zkStark = function () {
  ...
  return {
     ...
     */
    prove(computation, inputs, privateIndices) {
...
zeroKnowledgeProofs.setMembership = function (set) {
  ...
  return {
    ...
    proveInSet(element) {
      ...
      const elementIndex = set.findIndex((item) => JSON.stringify(item) === JSON.stringify(element));
