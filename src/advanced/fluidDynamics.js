...
fluidDynamics.pressureDrop = function (frictionFactor, length, diameter, density, velocity) {
  return frictionFactor * (length / diameter) * (density * velocity ** 2) / 2;
};
...
fluidDynamics.frictionFactor = function (reynoldsNumber, relativeRoughness) {
  ...
  let f = (-1.8 * Math.log10((relativeRoughness / 3.7) ** 1.11 + 6.9 / reynoldsNumber)) ** -2;
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    f -= (1 / Math.sqrt(f) + 2 * Math.log10(term)) / (df);
