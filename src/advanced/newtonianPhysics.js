...
newtonianPhysics.projectileMotion = function(params, time) {
  const { initialHeight, initialVelocity, angle, gravity = 9.81 } = params;
...
newtonianPhysics.circularOrbit = function(mass1, mass2, radius) {
  ...
  return { velocity, period, force, angularMomentum };
};
...
newtonianPhysics.simulateMotion = function(initialState, mass, forceField, options = {}) {
  ...
  for (let i = 1; i <= steps; i++) {
    ...
    if (method === 'euler') {
      ...
      newPosition = prevState.position.map((p, j) => 
        p + prevState.velocity[j] * timeStep + 0.5 * acceleration[j] * timeStep * timeStep
      );
      
      const newForce = forceField(newPosition, prevState.velocity, t);
      const newAcceleration = newForce.map(f => f / mass);
      
      newVelocity = prevState.velocity.map((v, j) => 
        v + 0.5 * (acceleration[j] + newAcceleration[j]) * timeStep
      );
    }
    else if (method === 'rk4') {
      ...
      newPosition = prevState.position.map((p, j) => 
        p + (k1p[j] + 2*k2p[j] + 2*k3p[j] + k4p[j]) * timeStep / 6
      );
      
      newVelocity = prevState.velocity.map((v, j) => 
        v + (k1v[j] + 2*k2v[j] + 2*k3v[j] + k4v[j]) * timeStep / 6
      );
    }
...
newtonianPhysics.rotationalMotion = function(params, time) {
  const { inertia, torque, initialAngularVelocity, initialAngle } = params;
