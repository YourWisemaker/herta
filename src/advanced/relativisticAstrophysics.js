...
relativisticAstrophysics.constants = {
  ...
  schwarzschildRadius: function(mass) {
...
relativisticAstrophysics.gravitationalWaveFrequency = function(mass1, mass2, separation) {
  ...
  const orbitalFreq = (1 / (2 * Math.PI)) * Math.sqrt(
    this.constants.G * totalMass / Math.pow(separation, 3)
  );
...
relativisticAstrophysics.gravitationalWaveStrain = function(mass1, mass2, distance, frequency) {
  const chirpMass = Math.pow(mass1 * mass2, 3/5) / Math.pow(mass1 + mass2, 1/5);
  
  // Calculate strain amplitude
  const strain = (4 / distance) * 
                Math.pow(this.constants.G * chirpMass / Math.pow(this.constants.c, 2), 5/3) * 
                Math.pow(Math.PI * frequency, 2/3);
...
relativisticAstrophysics.kerrMetric = function(mass, spin, r, theta) {
  ...
  const rho2 = r * r + a * a * Math.pow(Math.cos(theta), 2);
  const sigma = Math.pow((r * r + a * a), 2) - a * a * delta * Math.pow(Math.sin(theta), 2);
  
  // Metric components (in Boyer-Lindquist coordinates)
  const gtt = -(1 - rs * r / rho2);
  const gtphi = -rs * r * a * Math.pow(Math.sin(theta), 2) / rho2;
  const grr = rho2 / delta;
  const gthth = rho2;
  const gphiphi = (r * r + a * a + rs * r * a * a * Math.pow(Math.sin(theta), 2) / rho2) * 
...
relativisticAstrophysics.effectivePotential = function(mass, spin, particleE, particleL, r) {
  ...
  const V = -1 + Math.pow(particleE * (r * r + a * a) - particleL * a, 2) / 
           (r * r * delta) - 
           delta * (1 + Math.pow(particleL - particleE * a, 2) / r / r) / r / r;
...
relativisticAstrophysics.blackHoleEntropy = function(mass) {
  ...
  const area = 4 * Math.PI * Math.pow(this.schwarzschildRadius(mass), 2);
...
relativisticAstrophysics.frameDraggingPrecession = function(mass, spin, radius, inclination) {
  ...
  return (2 * this.constants.G * J) / 
         (this.constants.c * this.constants.c * Math.pow(radius, 3)) * 
...
relativisticAstrophysics.gravitationalWaveLuminosity = function(mass1, mass2, separation) {
  ...
  // Orbital frequency
  const omega = Math.sqrt(this.constants.G * totalMass / Math.pow(separation, 3));
  
  // Quadrupole formula for GW luminosity
  return (32/5) * Math.pow(this.constants.G, 4/3) * 
         Math.pow(mu, 2) * Math.pow(totalMass, 2/3) * 
         Math.pow(omega, 10/3) / 
         Math.pow(this.constants.c, 5);
};
...
relativisticAstrophysics.geodeticPrecession = function(centralMass, semiMajorAxis, eccentricity) {
  // Orbital frequency
  const omega = Math.sqrt(this.constants.G * centralMass / Math.pow(semiMajorAxis, 3));
