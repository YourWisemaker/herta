...
computerVision.sobelEdgeDetection = function (image) {
  ...
  return {
    gradientX, gradientY, magnitude, direction
  };
};
...
computerVision.houghCircles = function (image, minRadius = 10, maxRadius = 50, threshold = 30) {
  ...
  const accumulator = Array(height).fill().map(() => Array(width).fill().map(() => Array(radiusRange).fill(0)));
...
computerVision.segmentImage = function (image, k = 5) {
  ...
  const segmented = Array(height).fill().map(() => Array(width).fill().map(() => [0, 0, 0]));
