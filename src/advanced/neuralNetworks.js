...
neuralNetworks.denseLayer = function(inputSize, outputSize, options = {}) {
  ...
  return {
    ...
    update: function(newWeights, newBias) {
      if (newWeights) {
        weights = newWeights;
      }
...
neuralNetworks.convLayer2d = function(options = {}) {
  ...
  const kernels = Array(outputChannels).fill().map(() => 
    Array(inputChannels).fill().map(() => 
      matrix.random([kernelSize, kernelSize], -scale, scale)
    )
  );
  ...
  return {
    ...
    forward: function(input) {
      ...
      // Initialize output
      const output = Array(batchSize).fill().map(() => 
        Array(outputChannels).fill().map(() => 
          Array(outputHeight).fill().map(() => 
            new Array(outputWidth).fill(0)
          )
        )
      );
    ...
    update: function(newKernels, newBias) {
      if (newKernels) {
        kernels = newKernels;
      }
...
neuralNetworks.rnnLayer = function(inputSize, hiddenSize, options = {}) {
  ...
  return {
    ...
    // Forward pass
    forward: function(input, initialHidden = null) {
      ...
      for (let t = 0; t < seqLength; t++) {
        ...
        hidden = xh.map((row, i) => 
          row.map((val, j) => val + hh[i][j] + (useBias ? bh[j] : 0))
        );
    ...
    // Update layer parameters
    update: function(newWxh, newWhh, newBh) {
...
neuralNetworks.lstmLayer = function(inputSize, hiddenSize, options = {}) {
  ...
  return {
    ...
    // Forward pass
    forward: function(input, initialHidden = null, initialCell = null) {
      // Input shape: [batchSize, sequenceLength, inputSize]
      const batchSize = input.length;
      const seqLength = input[0].length;
      ...
      for (let t = 0; t < seqLength; t++) {
        ...
        const i = xi.map((row, i) => row.map((val, j) => 
          machineLearning.sigmoid(val + hi[i][j] + (useBias ? bi[j] : 0))
        ));
        
        // Forget gate
        const xf = matrix.multiply(xt, wxf);
        const hf = matrix.multiply(hidden, whf);
        const f = xf.map((row, i) => row.map((val, j) => 
          machineLearning.sigmoid(val + hf[i][j] + (useBias ? bf[j] : 0))
        ));
        
        // Cell gate
        const xc = matrix.multiply(xt, wxc);
        const hc = matrix.multiply(hidden, whc);
        const cHat = xc.map((row, i) => row.map((val, j) => 
          Math.tanh(val + hc[i][j] + (useBias ? bc[j] : 0))
        ));
        
        // Update cell state
        cell = cell.map((row, i) => row.map((val, j) => 
          f[i][j] * val + i[i][j] * cHat[i][j]
        ));
        
        // Output gate
        const xo = matrix.multiply(xt, wxo);
        const ho = matrix.multiply(hidden, who);
        const o = xo.map((row, i) => row.map((val, j) => 
          machineLearning.sigmoid(val + ho[i][j] + (useBias ? bo[j] : 0))
        ));
        
        // Update hidden state
        hidden = cell.map((row, i) => row.map((val, j) => 
          o[i][j] * Math.tanh(val)
        ));
